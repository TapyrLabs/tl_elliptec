...
```{important}
**`stop()` does not interrupt a bounded `move_absolute`/`move_relative`/
`home`** once issued — the move runs to completion regardless of calling
`stop()`. It only stops continuous jog motion (jog step size 0) or aborts
an in-progress `optimize_motors`/`clean_mechanics` cycle.
```
